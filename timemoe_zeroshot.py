...
DEFAULT_MODEL_ID = "Maple728/TimeMoE-50M"
...
def decode_prediction(output: torch.Tensor, horizon: int) -> np.ndarray:
    if output.ndim < 2:
        raise ValueError(f"Expected output rank >= 2, got shape {tuple(output.shape)}")
    ...
    if pred.ndim == 3 and pred.shape[-1] == 1:
        pred = pred[..., 0]
    if pred.ndim != 2:
        raise ValueError(f"Expected 2D predictions, got shape {tuple(pred.shape)}")
...
def forecast_batches(
    ...
    batch_size: int,
    device: torch.device,
    normalize_per_context: bool,
) -> np.ndarray:
    if not contexts:
        return np.empty((0, horizon), dtype=np.float32)

    preds: List[np.ndarray] = []
    model_dtype = model.dtype if isinstance(model.dtype, torch.dtype) else torch.float32
    model_dtype = model_dtype if model_dtype.is_floating_point else torch.float32

    for i in range(0, len(contexts), batch_size):
        batch = np.stack(contexts[i : i + batch_size]).astype(np.float32)
        seqs = torch.from_numpy(batch).to(device=device, dtype=model_dtype)

        if normalize_per_context:
            mean = seqs.mean(dim=-1, keepdim=True)
            std = seqs.std(dim=-1, keepdim=True)
            std = torch.where(std < 1e-6, torch.ones_like(std), std)
            model_inputs = (seqs - mean) / std
        else:
            mean, std = None, None
            model_inputs = seqs

        with torch.no_grad():
            output = model.generate(model_inputs, max_new_tokens=horizon)
        batch_preds = decode_prediction(output, horizon)
...
def evaluate_dataset(
    ...
    normalize_per_context = normalization_mode == "context"
    ...
    for col in target_columns:
        ...
        preds = forecast_batches(
            ...
            device=device,
            normalize_per_context=normalize_per_context,
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Time-MoE zero-shot evaluation on ETT datasets."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="train",
        choices=["train", "context"],
        help=(
            "train: use train-split standardization like Time-MoE benchmark eval; "
            "context: normalize each input context and inverse predictions (README example)."
        ),
    )
    ...
    if args.context_length + max_pred_len > 4096:
        print(
            "Warning: context_length + prediction_length exceeds 4096, "
            "which is above Time-MoE's README recommendation."
        )
